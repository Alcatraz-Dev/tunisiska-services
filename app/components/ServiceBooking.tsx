...
export default function ServiceBooking({
  ...
  if (showPayment) {
    ...
    return (
      <View className="flex-1">
        <AutoText
          className={`text-center text-lg font-bold mb-4 ${isDark ? "text-white" : "text-black"}`}
        >
          Slutför betalning
        </AutoText>
        <TouchableOpacity
  ...
  return (
    <ScrollView className="flex-1 p-6">
      ...
      {/* Service-specific fields */}
      <AutoText
        className={`text-lg font-bold mb-4 mt-6 ${isDark ? "text-white" : "text-gray-900"}`}
      >
        Tjänstedetaljer
      </AutoText>
          ...
          <>
            ...
            {pointsToUse > 0 && (
              <View className="flex-row justify-between mb-2">
                ...
                <AutoText className="text-green-500">
                  -{(pointsToUse / 10).toFixed(0)} SEK
                </AutoText>
              </View>
            )}
          </>

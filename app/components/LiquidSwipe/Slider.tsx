...
const Slider = ({
  ...
  // ⬅️ Pan gesture الجديد
  const pan = Gesture.Pan()
    .onBegin((e) => {
      if (e.x <= MARGIN_WIDTH && hasPrev) {
        activeSide.value = Side.LEFT;
        zIndex.value = 100;
      } else if (e.x >= WIDTH - MARGIN_WIDTH && hasNext) {
    ...
    .onUpdate((e) => {
      if (activeSide.value === Side.LEFT) {
        left.x.value = Math.max(e.x, MARGIN_WIDTH);
        left.y.value = e.y;
      } else if (activeSide.value === Side.RIGHT) {
        right.x.value = Math.max(WIDTH - e.x, MARGIN_WIDTH);
        right.y.value = e.y;
      }
    })
    .onEnd((e) => {
      if (activeSide.value === Side.LEFT) {
        const dest = snapPoint(e.x, e.velocityX, LEFT_SNAP_POINTS);
        ...
        left.y.value = withSpring(HEIGHT / 2, { velocity: e.velocityY });
      } else if (activeSide.value === Side.RIGHT) {
        const dest = snapPoint(e.x, e.velocityX, RIGHT_SNAP_POINTS);
        ...
        right.y.value = withSpring(HEIGHT / 2, { velocity: e.velocityY });
      }
    });
